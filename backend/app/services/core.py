"""
core.py — Todos los servicios del backend (DataService, RiskCalculator, etc.)
Originalmente en app/services.py — movido aquí para respetar la estructura de paquetes.
"""
import time
import logging
import functools
...
class DataService:
    @timer_log
    def get_prices(self, ticker: str, years: int = None,
        ...
        if start and end:
            ...
            df = yf.download(ticker, period=f"{y}y",
                             auto_adjust=True, progress=False)
        if df.empty:
...
class RiskCalculator:
    ...
    def returns_stats(self, df: pd.DataFrame) -> Dict:
        ...
        sh_stat, sh_pval = stats.shapiro(
            ret_log[-500:] if len(ret_log) > 500 else ret_log
        )
    ...
    @timer_log
    def compute_var(self, df: pd.DataFrame,
                    confidence: float, simulations: int) -> Dict:
        close = df["Close"].dropna()
        ...
        for name, sp in specs.items():
            try:
                ...
                fcast_vol = (
                    ...
                    if forecast.variance is not None else []
                )
                results[name] = {
                    "aic":        round(r.aic, 2),
                    "bic":        round(r.bic, 2),
                    ...
                    "forecast_5d": _clean(fcast_vol.tolist())
                                   if hasattr(fcast_vol, "tolist") else [],
...
class PortfolioAnalyzer:
    ...
    def capm(self, start: str = None, end: str = None) -> List[Dict]:
        ...
        rf_ticker = yf.download("^IRX", period="5d", progress=False,
                                auto_adjust=True)
        rf_annual  = float(rf_ticker["Close"].iloc[-1].squeeze()) / 100 \
                     if not rf_ticker.empty else 0.05
...
class AlertasService:
    ...
    def generate(self, ticker: str, df: pd.DataFrame) -> Dict:
        ...
        rsi_signal   = ("Sobrecompra" if rsi_val and rsi_val > 70
                        else "Sobreventa" if rsi_val and rsi_val < 30
                        else "Neutral")
        macd_signal  = ("Compra" if macd_val and macd_sig and macd_val > macd_sig
                        else "Venta")
        bb_signal    = ("Sobrecompra" if close_val and bb_up and close_val > bb_up
                        else "Sobreventa" if close_val and bb_lo and close_val < bb_lo
                        else "Neutral")
        sma_cross    = ("Compra" if sma_val and ema_val and ema_val > sma_val
                        else "Venta")
        stoch_signal = ("Sobrecompra" if stoch_k and stoch_k > 80
                        else "Sobreventa" if stoch_k and stoch_k < 20
                        else "Neutral")

        signals   = [rsi_signal, macd_signal, bb_signal, sma_cross, stoch_signal]
        buy_n     = sum(1 for s in signals if s in ["Compra",  "Sobreventa"])
        sell_n    = sum(1 for s in signals if s in ["Venta",   "Sobrecompra"])
        overall   = ("🟢 Compra" if buy_n > sell_n
                     else "🔴 Venta" if sell_n > buy_n
                     else "🟡 Neutral")
        ...
        def fetch(ticker, col="Close"):
            try:
                df = yf.download(ticker, period="5d", progress=False,
                                 auto_adjust=True)
                return float(df[col].iloc[-1].iloc[0]) if hasattr(df[col].iloc[-1], "iloc") else float(df[col].iloc[-1])
            except Exception:
                return 0.0
