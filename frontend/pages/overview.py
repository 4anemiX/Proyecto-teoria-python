...
def render():
    ...
    st.markdown(
        ...
        f'<span style="color:#2E3550;">({delta_days:,} días)</span>'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    <div class="interpretation-box {sentiment_class}" style="margin-top:16px;">
        ...
        {"Una mayoría alcista sugiere momentum positivo en el sector."
         if bullish > bearish else
         "La divergencia entre activos sugiere rotación sectorial o factores idiosincráticos."
         if bullish != bearish else
    ...
    st.markdown(
        ...
        f'text-transform:uppercase;color:#3B4460;margin-bottom:12px;">'
        f'Rendimiento Normalizado — Base 100 · {start_str} a {end_str}</div>',
        unsafe_allow_html=True,
    )
    ...
    if all_data:
        ...
        <div class="interpretation-box">
            <strong>Mejor desempeño:</strong> {best_ticker} con {final_values[best_ticker]:.1f}
            (base 100) — retorno acumulado de {final_values[best_ticker]-100:+.1f}%.
            ...
            {"indicando generación de alpha en el portafolio."
    ...
    st.markdown(
        ...
        'text-transform:uppercase;color:#3B4460;margin-bottom:12px;">Matriz de Correlaciones</div>',
        unsafe_allow_html=True,
    )
    ...
    if prices:
        ...
        fig2.update_layout(
            height=340,
            title="Correlaciones de rendimientos logarítmicos diarios",
        )
